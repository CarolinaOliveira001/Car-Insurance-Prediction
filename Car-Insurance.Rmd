...
```{r}
oldANNUAL <- carDf$ANNUAL_MILEAGE
...
carDf$VEHICLE_TYPE <- as.numeric(carDf$VEHICLE_TYPE)
carDf$ANNUAL_MILEAGE <- as.numeric(carDf$ANNUAL_MILEAGE)
```
...
```{r}
newDf <- SMOTE(train[c(1:17, 19)], train[18], K = 5, dup_size = 0)
```
...
```{r}
unique(smoteDf$GENDER)
```


```{r}
smoteDf$AGE <- factor(smoteDf$AGE)
smoteDf$GENDER <- factor(smoteDf$GENDER)
smoteDf$RACE <- factor(smoteDf$RACE)
smoteDf$DRIVING_EXPERIENCE <- factor(smoteDf$DRIVING_EXPERIENCE)
smoteDf$EDUCATION <- factor(smoteDf$EDUCATION)
smoteDf$INCOME <- factor(smoteDf$INCOME)
smoteDf$VEHICLE_YEAR <- factor(smoteDf$VEHICLE_YEAR)
smoteDf$VEHICLE_TYPE <- factor(smoteDf$VEHICLE_TYPE)
smoteDf$ANNUAL_MILEAGE <- factor(smoteDf$ANNUAL_MILEAGE)
```
...
```{r}
rf <- randomForest(as.factor(class) ~ ., data=smoteDf, proximity=TRUE) 
print(rf)
```
